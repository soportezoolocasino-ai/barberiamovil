...
router.get('/:id', async (req, res) => {
  try {
    ...
    const reviews  = await pool.query(
      `SELECT r.*, u.name as client_name FROM reviews r
       JOIN users u ON r.client_id = u.id
       WHERE r.barber_id=$1 ORDER BY r.created_at DESC LIMIT 10`,
      [req.params.id]
    );
