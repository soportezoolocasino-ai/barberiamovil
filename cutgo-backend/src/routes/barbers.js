...
router.get('/', async (req, res) => {
  try {
    const { lat, lng, radius = 10000 } = req.query; // 10000 km = sin límite práctico
    let query, params;
    if (lat && lng) {
      query = `
...
router.get('/:id/services', async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT id, name, price, duration_min as duration, icon FROM barber_services WHERE barber_id=$1 ORDER BY created_at ASC`,
      [req.params.id]
    );
    res.json({ services: rows });
  } catch (e) {
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const services = await pool.query(
      'SELECT id, name, price, duration_min as duration, icon FROM barber_services WHERE barber_id=$1',
