 ...
 * Con guardado real en el backend (servicios, perfil, horario)
 */
  ...
  try {
    ...
    if (minus24 > new Date()) {
      await Notifications.scheduleNotificationAsync({
        content: { title: '💈 Cita mañana', body: `${booking.client_name} — ${booking.service_names} a las ${booking.time}`, data: { bookingId: booking.id } },
    ...
    if (minus2 > new Date()) {
      await Notifications.scheduleNotificationAsync({
        content: { title: '⏰ Cita en 2 horas', body: `${booking.client_name} llega a las ${booking.time} — ${booking.service_names}`, data: { bookingId: booking.id } },
        trigger: { date: minus2 },
      });
    }
...
export const BarberHomeScreen = ({ navigation }) => {
  ...
  const advanceJob = async (nextStatus) => {
    ...
    else setActiveJob((j) => ({ ...j, status: nextStatus }));
  };
  ...
  const blockFullDay = (day) => {
    Alert.alert(`Bloquear ${day} completo`, 'Nadie podrá reservar este día.', [
  ...
  const saveSchedule = async () => {
    ...
    try {
      await barbersAPI.updateSchedule(schedule);
      setShowSchedule(false);
      setBlockMode(false);
      Alert.alert('✅', 'Horario guardado correctamente');
    } catch (e) {
  ...
  const saveShopData = async () => {
    ...
    try {
      ...
      for (const svc of shopData.services) {
        ...
        if (isNew) {
          await barbersAPI.createService({
            name:         svc.name,
            price:        svc.price,
  ...
  const saveSvc = () => {
    ...
    setShopData((prev) => ({
      ...prev,
      services: prev.services.find((s) => s.id === editingSvc.id)
        ? prev.services.map((s) => s.id === editingSvc.id ? editingSvc : s)
        : [...prev.services, editingSvc],
    }));
    setEditingSvc(null);
  };
  ...
  const net   = parseFloat(earningsData?.net_earnings || earningsData?.total || 0).toFixed(2);
  const gross = parseFloat(earningsData?.gross_total  || earningsData?.total || 0).toFixed(2);
  const cuts  = parseInt(earningsData?.total_cuts     || earningsData?.bookings || 0);
      ...
      <Modal visible={showSchedule} transparent animationType="slide">
        ...
        <View style={[styles.sheet, { maxHeight: '90%' }]}>
          ...
          <View style={styles.blockModeRow}>
            ...
            <Switch value={blockMode} onValueChange={setBlockMode} trackColor={{ false: colors.mid, true: '#c0392b' }} thumbColor={colors.white} />
          </View>
          <ScrollView showsVerticalScrollIndicator={false}>
            {DAYS.map((day) => (
              <View key={day}>
                  ...
                  {schedule[day].active ? (
                    <View style={styles.schedHours}>
                      ...
                      {schedule[day].blockedSlots?.length > 0 && (
                        <View style={styles.blockedPill}><Text style={styles.blockedPillText}>{schedule[day].blockedSlots.length} bloq.</Text></View>
                      )}
                    </View>
                  ) : (
                ...
                {editDay === day && editField && (
                  <View style={styles.hourPicker}>
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                      {HOURS.map((h) => (
                        <TouchableOpacity key={h} style={styles.hourOpt} onPress={() => { setSchedule((p) => ({ ...p, [day]: { ...p[day], [editField]: h } })); setEditDay(null); setEditField(null); }}>
                ...
                {blockMode && schedule[day].active && (
                  <View style={styles.slotGrid}>
                    {HOURS.filter((h) => h >= schedule[day].from && h <= schedule[day].to).map((slot) => {
                      ...
                      return (
                        <TouchableOpacity key={slot} style={[styles.slotChip, blocked && styles.slotChipBlocked]} onPress={() => toggleBlockSlot(day, slot)}>
          ...
          <TouchableOpacity style={styles.saveBtn} onPress={saveSchedule} disabled={savingSchedule}>
            {savingSchedule ? <ActivityIndicator color={colors.black} /> : <Text style={styles.saveBtnText}>Guardar horario</Text>}
          </TouchableOpacity>
              ...
              <View style={styles.svcEditCard}>
                ...
                <View style={{ flexDirection: 'row', gap: spacing.sm, marginTop: spacing.sm }}>
                  <View style={{ flex: 1 }}>
                    ...
                    <TextInput style={styles.panelInput} value={String(editingSvc.price || '')} onChangeText={(v) => setEditingSvc((s) => ({ ...s, price: parseFloat(v) || 0 }))} keyboardType="numeric" placeholderTextColor={colors.gray} placeholder="0" />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.panelFieldLabel}>Duración (min)</Text>
                    <TextInput style={styles.panelInput} value={String(editingSvc.duration || editingSvc.duration_min || '')} onChangeText={(v) => setEditingSvc((s) => ({ ...s, duration: parseInt(v) || 0 }))} keyboardType="numeric" placeholderTextColor={colors.gray} placeholder="30" />
                  </View>
                </View>
                <View style={{ flexDirection: 'row', gap: spacing.sm, marginTop: spacing.sm }}>
