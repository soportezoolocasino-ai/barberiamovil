    ...
    if (lat && lng) {
      query = `
        ...
        WHERE b.is_online = true
          AND (6371 * acos(cos(radians($1)) * cos(radians(b.lat)) *
          cos(radians(b.lng) - radians($2)) +
...
router.get('/me/services', auth, async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT id, name, price, duration_min, icon FROM barber_services WHERE barber_id=$1 ORDER BY created_at ASC`,
      [req.user.id]
    );
...
router.post('/me/services', auth, async (req, res) => {
  const { name, price, duration_min = 30, icon = 'C' } = req.body;
  if (!name || !price) return res.status(400).json({ error: 'Nombre y precio obligatorios' });
  try {
    const { rows } = await pool.query(
      `INSERT INTO barber_services (barber_id, name, price, duration_min, icon) VALUES ($1,$2,$3,$4,$5) RETURNING *`,
...
router.patch('/me/services/:id', auth, async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `UPDATE barber_services SET name=COALESCE($1,name), price=COALESCE($2,price), duration_min=COALESCE($3,duration_min), icon=COALESCE($4,icon) WHERE id=$5 AND barber_id=$6 RETURNING *`,
...
router.post('/me/blocks', auth, async (req, res) => {
  ...
  try {
    await pool.query(`INSERT INTO barber_blocks (barber_id, date, time) VALUES ($1,$2,$3) ON CONFLICT DO NOTHING`, [req.user.id, date, time]);
...
router.delete('/me/blocks', auth, async (req, res) => {
  const { date, time } = req.body;
  try {
    await pool.query(`DELETE FROM barber_blocks WHERE barber_id=$1 AND date=$2 AND time=$3`, [req.user.id, date, time]);
    res.json({ ok: true });
  } catch (e) {
...
router.get('/me/profile', auth, async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT u.id, u.name AS shop_name, u.address, u.phone, u.bio, u.photos, COALESCE(json_agg(bs ORDER BY bs.created_at) FILTER (WHERE bs.id IS NOT NULL),'[]') AS services FROM users u LEFT JOIN barber_services bs ON bs.barber_id = u.id WHERE u.id=$1 GROUP BY u.id`,
      [req.user.id]
    );
...
router.patch('/me/profile', auth, async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `UPDATE users SET bio=COALESCE($1,bio), address=COALESCE($2,address), phone=COALESCE($3,phone), photos=COALESCE($4,photos) WHERE id=$5 RETURNING id, name, bio, address, phone, photos`,
...
router.get('/:id/services', async (req, res) => {
  try {
    const { rows } = await pool.query(`SELECT id, name, price, duration_min, icon FROM barber_services WHERE barber_id=$1 ORDER BY created_at ASC`, [req.params.id]);
    res.json({ services: rows });
  } catch (e) {
    res.status(500).json({ error: 'Error al obtener servicios' });
  }
...
router.get('/:id/profile', async (req, res) => {
  try {
    ...
    if (!rows.length) return res.status(404).json({ error: 'Barberia no encontrada' });
    res.json({ profile: rows[0] });
  } catch (e) {
    res.status(500).json({ error: 'Error al obtener perfil' });
  }
...
router.get('/:id', async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT b.*, u.name, u.email, u.phone, u.avatar_url FROM barbers b JOIN users u ON b.user_id = u.id WHERE b.id=$1`,
      [req.params.id]
    );
    if (!result.rows.length) return res.status(404).json({ message: 'Barbero no encontrado' });
    const services = await pool.query('SELECT * FROM barber_services WHERE barber_id=$1', [result.rows[0].user_id]);
    const reviews = await pool.query(`SELECT r.*, u.name as client_name FROM reviews r JOIN users u ON r.client_id = u.id WHERE r.barber_id=$1 ORDER BY r.created_at DESC LIMIT 10`, [req.params.id]);
